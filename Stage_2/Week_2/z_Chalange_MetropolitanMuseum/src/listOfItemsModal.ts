...
function searchItemsByCulture() {
    ...
    searchBtn?.addEventListener("click", () => {
        const cultureInput = modal.querySelector("#cultureInput") as HTMLInputElement;
...
async function loadCulture(cultureValue: string) {

    const resultsContainer = document.querySelector(".culture-results") as HTMLElement;
    resultsContainer.innerHTML = "Loading...";

    try {

        const searchResponse = await fetch(
            `https://collectionapi.metmuseum.org/public/collection/v1/search?hasImages=true&q=${cultureValue}`
        );
        ...
        const cultureMatches = objects.filter(obj =>
            obj.culture &&
...
function renderCultureResults(results: any[], cultureValue: string) {
